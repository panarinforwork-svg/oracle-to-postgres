...
if [ "$1" = 'ora2pg' ]; then
    ...
    # Проверяем, есть ли переменная COMAND
    if [ -n "$COMAND" ]; then
        echo "INFO: COMAND variable detected: '$COMAND'"
        echo "INFO: Executing custom command..."
        # Выполняем команду из переменной COMAND
        eval "$COMAND"
    elif [ -z "$2" ]; then
        echo "INFO: no args provided and no COMAND variable. Using default: '--debug -c $CONFIG_LOCATION --basedir $OUTPUT_LOCATION ${ORA_HOST_FLAG} ${ORA_USER_FLAG} ${ORA_PWD_FLAG_MASKED}'"
        # Разбиваем ADDITIONAL_PARAMETRS на отдельные аргументы
        IFS=' ' read -r -a additional_params <<< "$ADDITIONAL_PARAMETRS"
        ora2pg --debug "${additional_params[@]}" -c "$CONFIG_LOCATION" --basedir "$OUTPUT_LOCATION" ${ORA_HOST_FLAG} ${ORA_USER_FLAG} ${ORA_PWD_FLAG}
    else
        echo "INFO: executing: '$@'"
        exec "$@"
    fi
    ...
    echo "INFO: Command completed. Container will stay alive for debugging."
    tail -f /dev/null
    exit 0
fi
